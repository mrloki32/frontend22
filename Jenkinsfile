            ...
            steps {
                script {
                    // Fetch the latest Docker image URI from ECR
                     sh(script: "aws ecr describe-images --repository-name demo-jen --query 'sort_by(imageDetails,& imagePushedAt)[-1].imageTags[0]' --output text", returnStdout: true).trim()

                    // Create a new task definition revision with the updated image
                     sh(script: "[[ -n '${ecrImageUri}' ]] && aws ecs register-task-definition --family jenkins-demo --container-definitions '[{\"name\":\"Angular\",\"image\":\"${ecrImageUri}\"}]' --query 'taskDefinition.taskDefinitionArn' --output text", returnStdout: true).trim()
